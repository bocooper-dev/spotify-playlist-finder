...
export class Validator {
  ...
  async validate(
    ...
    const sanitized: unknown = options.sanitize ? {} : undefined
  ...
  private generateCacheKey(data: unknown, schema: ValidationSchema): string {
...
export const SpotifySchemas = {
  searchRequest: {
    ...
    minFollowers: {
      ...
      optional: true,
      transform: (value: unknown) => value ? parseInt(value, 10) : undefined
    },
    maxFollowers: {
      rules: [Rules.number({ min: 0, max: 100000000, integer: true })],
      optional: true,
  ...
  exportRequest: {
    ...
    data: {
      rules: [
        Rules.async(async (data: unknown) => {
...
export class ClientValidator {
   ...
   */
  static async validateSearchForm(data: unknown): Promise<{
    valid: boolean
   ...
   */
  static async validateExportOptions(data: unknown): Promise<{
    valid: boolean
    errors: Record<string, string>
  }> {
    try {
